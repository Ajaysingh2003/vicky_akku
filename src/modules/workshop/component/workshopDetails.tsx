...
function WorkshopDetails({ workshop }: { workshop: getWorkshopId }) {
  ...
  console.log(workshop.eventDate);
  const formattedDate = format(
    workshop.eventDate,
    "do MMM yyyy | h:mm a",
  ).toUpperCase();
  if (!workshop) return <p className="bg-red-400 h-36reg">Not Found</p>;
    ...
    <div className="py-3 flex items-center justify-center  lg:py-7">
      <div className="w-full h-full grid grid-cols-1 gap-2   m-auto space-y-3 px-4">
        <div className="title-place flex-wrap order-1">
          <h3 className="text-4xl wrap-break-word  text-center md:text-left font-passion-one uppercase  text-[#4B4740]">
            {workshop.title}
          </h3>
        ...
        <div className="flex order-2 md:order-3 flex-col justify-center md:justify-normal items-center md:flex-row gap-3">
          <div className=" text-md rounded-sm  md:text-lg    lg:text-xl px-3 py-2 bg-[#F2E9F9] text-[#6B6B6B] w-fit flex items-center gap-3">
            <span className="inline-block font-normal">Date-</span>
            <h3 className="inline-block font-bold ">{formattedDate}</h3>
          </div>
          <div className="bg-[#F2E9F9] rounded-sm text-lg px-4 md:px-3 md:pr-8 py-2 w-fit text-[#6B6B6B] font-bold">
